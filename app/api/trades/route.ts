...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.MTN_TEST_ENVIRONMENT === 'true') {
      ...
      return NextResponse.json({ data: newTrade }, { status: 201 });
    }
...
export async function GET(request: Request) {
  try {
    if (process.env.MTN_TEST_ENVIRONMENT === 'true') {
      const seedTrades = require('../../../tests/e2e/fixtures/seed-trades.json');
      return NextResponse.json({ data: seedTrades.trades });
    }
