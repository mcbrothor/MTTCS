...
export async function GET(request: Request) {
  ...
  try {
    ...
    await upsertKrInvestorFlowDaily(getSupabaseAdmin(), rows);
    return apiSuccess({
      asOf: asOfDate,
      provider: collection.provider,
      quality: collection.errors.size === 0 ? 'FULL' : rows.length > 0 ? 'DEGRADED' : 'BLOCKED',
      modelVersion: 'kr-investor-flow-collection-v2',
      warnings: [...collection.errors].map(([ticker, message]) => `${ticker}: ${message}`),
      cursor,
    ...
    }, { asOf: asOfDate, provider: collection.provider, source: 'KIS KR investor flow batch' });
  } catch (error) {
