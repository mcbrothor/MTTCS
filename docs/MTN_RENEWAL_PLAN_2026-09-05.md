...
작성일: 2026-09-05 · 개정 5: 2026-09-09 실제 UI 기반 중복 화면·불필요 요소 최적화 보강

1차 기준: `1b29470` · 종가 추가 검토: 2026-09-05 20:49 KST · UI/성능 검토 중 로컬 HEAD: `86384d9`(21:15 KST) · 마지막으로 관측한 운영 SHA: `6f4c1c5`

**권고: 현재 다크 테마·가로 탐색·전문 표·큰 차트를 유지하면서 중복 조작과 과도한 상단 영역부터 정리하고, 동일 입력의 수집·계산·저장을 공유한다.** 기존 분석·리스크·성과 검증 자산과 7개 전략은 보존한다. 첫 2주에는 정확성·복구·성능 기준선을 정비하고, 이후 종목 상세를 중심으로 발굴부터 복기까지 연결한다.

개정 4는 로그인된 운영의 오늘·미너비니 스캐너·종목 상세/차트·종가베팅을 2026-09-09 직접 확인한 결과다. 앞선 밝은 화면·좌측 5개 메뉴 시안은 현재 구현과의 연속성이 부족하여 폐기한다. [실제 UI 기반 수정안과 이미지](/Users/mantori/vibecoding/MTN/docs/MTN_UI_REALISTIC_REVISION_2026-09-09.md)가 UI 전환 판단에서 우선한다. 위 SHA 및 아래 9월 5일 검증 수치는 역사적 기록이며 이번 운영 화면의 배포 지문이나 재실행 결과를 뜻하지 않는다.

개정 5는 같은 날짜 로컬 `c749667`에서 중복 탐색·조작·조회 사용처를 추가 대조했다. **복기 탭 단일화, 교차 검증 보기 흡수, 반복 링크/후보 버튼 정리, 종목 분석 조회 공유, 보조 정보 지연 조회, 미사용 코드 검증**을 13절과 상세안 8절 D01~D11에 편입했다. 새 문서를 별도로 늘리지 않고 기존 UP01~UP10의 구체적인 완료 조건으로 관리한다.
...
개정 3은 사용자 요청에 따라 화면 배치·주요 행동·모바일 사용성·34개 페이지의 통합/유지 지도와 성능 개선 순서를 구체화한다. 상세는 [UI·성능·기능 통합 설계안](/Users/mantori/vibecoding/MTN/docs/MTN_UI_PERFORMANCE_BLUEPRINT_2026-09-05.md)에서 관리한다. 검토 중 다른 작업이 기존 수정과 종가베팅 소스를 `86384d9`로 커밋하고 날짜 전환용 E2E를 추가했다. 이 개정에서 운영 성능이나 새 테스트의 통과를 실측한 것은 아니다.
...
| 영역 | 현재 확인 | 리뉴얼 판단 |
|---|---|---|
| 규모 | 페이지 파일 34개, API route 110개, 단위 테스트 파일 184개, E2E spec 38개, Supabase migration 91개 | 종가베팅과 redirect 페이지 포함. 개정 3 중 날짜 전환용 E2E spec 1개 추가 확인. |
...
| 사용자 흐름 | 1차 선택 E2E 32개 통과. 종가베팅 정적 렌더·조회 계약 테스트 통과, 이후 날짜 전환/오래된 응답/오류 재시도/timeout E2E 4개 추가 확인 | 이번 개정에서 새 E2E 실행은 하지 않았다. 날짜 전환 검증을 만료·철회·모바일·키보드·실제 DB 검증으로 확대하지 않는다. |
...
1차 전환은 현재 **오늘 · 시장 분석 · 종목 발굴 · 콘테스트 · 관심종목 · 매매 계획 · 포트폴리오 · 성과 복기**의 가로 메뉴와 별도 **투자 전략** 진입점을 유지한다. 종목 발굴 아래 6개 엔진 탭도 보존한다. 아래 오늘·시장·종목·운용·복기는 기능 소유권의 논리적 분류이며, 즉시 5개 메뉴로 합치는 구현 지시가 아니다. 메뉴 통합은 동일 업무의 사용성 비교 후 별도 결정한다.

기본 UI는 현재 네이비 배경·에메랄드 강조, 상단 로고/검색/시장 지표와 가로 탐색이다. 스캐너의 설명·설정·요약을 압축하여 전문 표를 먼저 보여주고, 콘테스트로 가는 중복 부유 버튼을 하나의 선택 행동 바로 통합한다. 전체 폭 표와 큰 차트를 유지하며 목록/상세 분할은 선택형으로 검증한다. 종가베팅은 두 시장 Top5와 데이터 상태를 앞세우고 익일 성과를 로컬 탭·요약으로 연결한다. 모바일 탐색·계좌 선택기 추가는 이번 데스크톱 관찰만으로 확정하지 않는다.
...
| 종목 | 6종 스캐너, 여러 스캐너 포착, 콘테스트, 관심종목, Stock360 | 전략 프리셋으로 후보 탐색→비교→종목 상세→관심등록/계획. 교차 확인은 포착 집계 필터, 콘테스트는 선택 후보의 심층 검토로 둔다. |
...
종목 상세는 공통 조회와 동작을 공유하되 현재 큰 MTN Pro 차트와 전문 지표를 보존한다. 상단에 종목·거래소·통화·분석 기준시각을 두고 핵심 지표·패턴 차트·조건별 근거·무효화·계획 행동으로 연결한다. 차트 버튼으로 진입하면 차트를 바로 보여준다. 쿨라매기의 snapshotId 기반 셋업 증거를 최신 Stock360 조회로 대체하지 않는다. 계획 작성 시 근거 ID와 데이터 기준시각을 전달한다.
...
- 시장·종목·계좌·필터를 URL과 공유 상태 계약으로 유지한다. 계좌는 불투명 ID와 권한 검사로 연결하고 자본·잔액·개인 메모는 URL에 넣지 않는다. 새로고침·뒤로가기·직접 링크에서도 같은 맥락이어야 한다.
...
성능은 **원천 스냅샷→공통 지표→전략 판정→계좌 리스크 계산**으로 분리하고 화면은 필요한 요약·상세만 읽도록 개선한다. 선택 상태 Context와 데이터 조회를 분리해 동일 macro 재조회, 계좌 금액별 원천 재수집, 중복 응답 직렬화를 줄인다. 기존 캐시·singleflight·batch 저장을 확장하고, 원 만료와 과거 근거를 보존한다. 서버 초기 조회·클라이언트 상호작용·차트 지연 로딩은 Next.js의 공식 [운영 체크리스트](https://nextjs.org/docs/app/guides/production-checklist)에 맞춰 검증한다. 적용 계층과 key·무효화·마감 우선순위는 상세안 4~5절을 따른다.

## 6. 실행 로드맵

기간은 **전담 개발자 1명과 AI 보조, 기존 인프라 유지, 주중 검증 가능**을 가정한 11~14주 추정이다. UI·성능 최적화는 기존 공통화/API 재구성 작업의 구체화로 편입하며 같은 작업을 중복 가산하지 않는다. 기존 수정·월간 공통 화면·캐시 구현도 재구현하지 않는다. 1주 차 기준선과 5주 차 시제품에서 범위·기간을 다시 산정한다. 인프라 교체나 증거 저장 구조의 전면 이관이 필요해지면 별도 산정하며, 2명이 병렬 구현해도 장중 관찰 기간은 유지한다.

| 단계 | 예상 시점 | 병렬 작업과 산출물 | 다음 단계 진입 조건 |
|---|---|---|---|
| 0. 기준선 | 1주 차 | 대표 5개 업무·34개 URL 소유권 / HAR·bundle·provider·DB·job 계측 / 운영 SHA·47개 작업·백업·기존 수정 검증 | 실제 환경/작업/백업 확인, 기준 스냅샷·성능 측정 조건 확보 |
| 1. 공통 기반 | 2~3주 차 | 선택/조회 key·원천 시각·세션·UI 상태 / 멱등 생성·원 만료 보존 / 발행 성공·소유권·복구·인증 | UI는 공통 계약 후 진행. 쓰기 전환은 중복 job 상태 보존·소유권·복원, 종가 전환은 시각·발행창 검증 |
| 2. 후보→계획 | 4~5주 차 | 기존 가로 탐색·6개 엔진 유지, 스캐너 상단 압축·단일 선택 행동 바·종가 후보/성과 배치 / 공유 조회·benchmark·계좌 계산 분리·지연 로딩 | 전문 열·큰 차트·맥락·근거·구형 링크 유지, 동일 조회/불필요한 재계산 제거, 첫 UI/성능 비교 후 재산정 |
| 3. 운용→복기 | 6~8주 차 | 대표 편집기·보유/복기 집계 조회 / 종가 철회·평가·누락 복구 / 7개 전략 공통 표현·마감 우선순위 | 실제/가상 분리, 평가 버전·기업행동 검증, 대량 기록·마감 용량 확인 |
| 4. 운영·전환 준비 | 9~10주 차 | 47개 작업·동시 마감 부하 / production E2E·접근성·HAR 비교 / 기능 중복·구형 URL 전수 점검 | 성능·정확성·신선도·중복 제거 동시 충족, 실패/skip 구분, 복귀 실습 |
...
UI·최적화 보강분은 상세안의 UP01~UP10으로 연결한다. 첫 2주에 UP01의 화면/조회/계산 소유권과 성능 기준선을 확보하고, UP02의 동일 job 생성 시 실행 상태 초기화·캐시 승격 시 만료 연장에 대한 재현/계약을 정한다. UP03/UP04의 대표 UI·조회 계층을 함께 설계한다. UP02의 작업 상태 보존은 R06 세부 항목 중 쓰기 전환 전 P0 조건으로 관리한다.
...
| 접근성 | 현재 가로 탐색·6개 스캐너 탭과 핵심 업무 화면·종목 상세·계획·종가베팅에서 serious/critical 자동 위반 0건, 키보드 초점·200% 확대·360px 검증 통과. 종가 익일 평가 표의 모바일 읽기 검증 포함. |
...
| 조회·계산 비용 | 저장된 요약/목록의 서버 처리 warm p95 300ms·cold p95 1.5초를 초기 목표로 한다. 공급자 갱신·브라우저 왕복은 별도 측정. 자본 변경에 따른 원천 재취득/공통 지표 재계산 0회, 초기 차트/운영 상세 미로딩. |
| 중복 제거 | 같은 key/세대의 브라우저 동시 조회 1개, 숨긴 탭 자동 polling 0건, 같은 생성 key의 중복 레코드·실행 job 초기화 0건. 대표 편집기 밖 독립 저장 경로 0개. 불변 근거·의도된 retry/정정은 보존. |
| UI 인수 | 오늘의 작업에서 담당 상세/편집기까지 화면 이동 3회 이하, 34개 기존 경로·주요 query 복원, 종목 목록/상세·모바일 뒤로가기 상태 유지. 전문 지표·6개 엔진·7개 전략 의미 보존. |
...
- 현재/신규 기능 소유권 지도, 화면별 UI 배치·주요 행동·이전 URL 연결표, 사용성 검토용 시안
- 공통 디자인·검색·상태 표현과 시장/계좌/종목 계약
- 데이터 카탈로그·API 계약·정책/모델 버전·재현 가능한 평가 기록
- 작업 목록·lease/재시도 정책·운영 화면·장애 대응 절차
- production 검증 결과, 성능 기준선/전후 비교·중복 요청/계산/저장 점검표, 백업 복원·릴리스·rollback 실습 기록
...
| review | 9월 5일 로컬 확인: 다음 거래일 개장30분 경과 후 가상 평가(서비스는 +32분에 데이터 확인) | 미평가/자료부족 backlog 복구, 가상 성과와 실제 계좌 복기 분리, 평가 버전/원천 hash 보존 |
| 익일 시초 성과 | 9월 9일 운영 UI 확인: 추천일 KRX 종가 → 다음 거래일 NXT 08:05 / KRX 09:05 표시 1분봉 종가, 왕복25bp 가정 | NXT 미거래·누락 미확인, 선정 0개는 미평가. +30분 평가와 정책 ID·가격 기준·비용을 분리하여 보존. 새 계산 구현 검증은 별도 |
...
## 12. UI·성능 최적화의 의사결정 기준

공통화 우선순위는 **같은 요청 제거 → 필요한 필드/구간만 조회 → 공통 계산 재사용 → 화면/차트 지연 로딩 → 마감 작업의 용량 배정 → 실측 후 저장 구조 개선**이다. 표면적인 메뉴 축소나 모든 데이터를 한 번에 불러오는 통합 대시보드를 완료로 보지 않는다.

상세안의 UP01~UP10과 인수 기준을 이 로드맵의 일부로 적용한다. 성능 수치는 기준선에서 확정하며, 캐시 TTL 연장·자료 기준 완화·과거 근거 생략으로 응답시간을 맞추지 않는다. 정확성·신선도·권한·마감 성공률과 사용성을 함께 충족한 화면부터 전환한다. UI 시안의 가상 데이터는 배치·작업 흐름 검토용이며 실제 추천·운영 성과를 뜻하지 않는다.

## 13. 중복 화면·불필요 요소의 추가 최적화

화면 수 감축과 처리 비용 감축을 따로 측정한다. 같은 기능의 별도 구현은 줄이고, 서로 다른 판단·저장 대상은 유지한다. 대상별 근거·이동 경로·보존 조건·인수 기준의 원본은 [UI·성능 상세안 8절](/Users/mantori/vibecoding/MTN/docs/MTN_UI_PERFORMANCE_BLUEPRINT_2026-09-05.md)에 둔다.

| 정리 범위 | 추가 결정 | 기존 작업 연결 |
|---|---|---|
| 화면·탭 | 복기 본문과 하위 탐색의 같은 view 전환을 한 곳으로 통합. 교차 검증은 종목 발굴의 ‘여러 스캐너 포착’ 보기로 흡수 | D01/D02 → UP03/UP04/UP09 |
| 반복 조작·큰 영역 | 오늘 하단 일반 경로 나열·중복 CTA 문구, 스캐너 이중 후보 버튼, 큰 설정 영역, 종가 미선정 반복 행 정리 | D03/D04/D05/D08 → UP03/UP06, CB03 |
| 상세·입력 | 관심 메모 편집과 분석 조회 분리, 종목 요약/차트/계획 진입 공유. 엔진별 근거와 전문 열 보존 | D05/D06 → UP03/UP04/UP06 |
| 부가 정보·조회 | 빈출 추천·장문 평가 설명은 필요한 위치에서 공개. 닫힌 패널·CSS로 숨긴 메뉴의 전용 조회 중단, 공통 시장 데이터 재사용 | D07/D11 → UP04/UP06/UP08 |
| 보조 탐색·잔여 코드 | 도움말/관리로 보조 링크 묶기 검증, 미참조 탐색 컴포넌트는 참조·빌드 확인 후 정리 | D09/D10 → UP09/UP10 |

실행은 **A: 화면 안의 반복 정리 → B: 보기·조회 통합 → C: 저빈도 탐색·잔여 코드 정리** 순으로 진행한다. A는 기존 2~5주 UI 작업의 첫 대상으로 하고, B는 공통 key·스냅샷 계약이 확인되는 범위부터 2~6주 조회 작업에 편입한다. C는 전환 관찰과 복귀 확인 후 진행한다. 기존 R/CB/UP와 동일 작업이므로 기간·비용을 다시 가산하지 않는다.

교차 보기는 한 스캐너 결과에 붙이는 단순 필터가 아니다. 6개 결과의 합집합·소스별 포함 규칙·기준시각·누락 상태를 보존한다. `/cross-check`는 동등성 확인 후 호환 경로로 남기며, 보기 진입만으로 전체 스캔을 자동 실행하지 않는다. 복기 탭도 기존 view·시장/유니버스·날짜를 보존한다.

관심종목/콘테스트, 계획/보유/실제 복기, 6개 엔진/7개 전략, 최신 분석/과거 스냅샷, 실제 체결/NXT·KRX 시초/+30분 가상 평가는 고유 기능으로 유지한다. 이미 공유하는 차트 컨테이너·전략 셸·월간 페이지와 기존 redirect를 신규 통합 성과로 중복 계산하지 않는다.

추가 완료 기준은 같은 view의 전환 탭1벌, 콘테스트 부유 행동 바1개, 메모 수정·보기 변경·정렬의 원천 재수집0회, 비표시 전용 패널 조회0회다. 기존 링크·선택·필터·과거 근거 보존과 동일 조건의 응답·전송량·오류율 비교를 함께 통과해야 한다. 이번 요청은 계획 보강이며 화면·코드 삭제나 기능 변경은 수행하지 않았다.


## 2026-09-09 구현 반영

UI·중복 기능 최적화 단계의 코드 적용 내역, 실제 렌더링 화면과 검증 결과는 [구현 결과](MTN_UI_IMPLEMENTATION_2026-09-09.md)에 정리했다. 기존 계획의 전체 백엔드 리뉴얼과 운영 성능 측정은 후속 범위이며, 이번 구현은 운영 배포 전 로컬 검증 기준이다.
