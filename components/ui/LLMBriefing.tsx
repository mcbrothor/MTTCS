...
function buildDefaultNarrative({
  ...
  if (isStale || state === 'GREY') {
    return {
      ...
      us:
        '현재 시장 내부 건강도 또는 시장 밖 위험 데이터가 완전하지 않습니다. 시장이 나쁘다는 뜻이 아니라 판단을 잠시 보류해야 하는 상태입니다.',
      kr:
        '한국 시장도 데이터 신뢰도 확인이 우선입니다. 지수·환율·수급 데이터가 정상화된 뒤 지수 평균선 위치, 시장 폭, 강한 업종을 다시 확인합니다.',
    };
  }
  ...
  if (state === 'RED') {
    return {
      ...
      us:
        '시장 내부 건강도가 위험 구간입니다. 시장 밖 흐름이 일부 우호적이어도 새 매수보다 현금 방어, 손절선 준수, 기존 포지션 축소 여부가 우선입니다.',
      kr:
  ...
  if (state === 'YELLOW') {
    return {
      ...
      us:
        `시장 내부 건강도가 아직 애매합니다${typeof score === 'number' ? ` · 종합 점수 ${score}/100` : ''}. 추세가 완전히 회복되기 전까지 새 매수는 보류하고 기존 포지션 관리에 집중합니다.`,
      kr:
        '한국 시장은 개별 주도주가 살아 있더라도 시장 폭과 외국인 수급 확인이 필요합니다. 종목 발굴은 가능하지만 매수 실행은 시장 내부 건강도 회복 이후로 미룹니다.',
    };
  }
  ...
  return {
    label: macroRegime ?? 'NEUTRAL',
    us:
      '시장 내부 건강도가 좋으면 종목 발굴을 진행할 수 있습니다. 다만 시장 밖 위험이 조심 구간이거나 애매하면 투자 비중과 추가 매수 속도를 낮춰야 합니다.',
    kr:
