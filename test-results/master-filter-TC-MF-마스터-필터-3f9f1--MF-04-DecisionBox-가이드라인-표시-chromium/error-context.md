...
- Name: master-filter.spec.ts >> TC-MF: 마스터 필터 >> 방어 (RED) 상태 >> MF-04: DecisionBox 가이드라인 표시
- Location: tests/e2e/master-filter.spec.ts:38:9
...
Locator: locator('text=현금 비중').or(locator('text=신규 매수 금지')).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('text=현금 비중').or(locator('text=신규 매수 금지')).first()
