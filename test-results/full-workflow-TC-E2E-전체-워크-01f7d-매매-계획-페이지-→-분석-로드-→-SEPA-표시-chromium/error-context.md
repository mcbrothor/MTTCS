...
- Name: full-workflow.spec.ts >> TC-E2E: 전체 워크플로우 통합 시나리오 >> 시나리오 A: 정상 흐름 (RISK_ON + 강한 후보) >> E2E-A04: 매매 계획 페이지 → 분석 로드 → SEPA 표시
- Location: tests/e2e/full-workflow.spec.ts:49:9
...
Call log:
  ...
  - waiting for locator('text=신규 매매 계획')
