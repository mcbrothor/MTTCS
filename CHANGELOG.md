...
### 주요 변경 사항
- **RS 소스 명확화**: `DB_BATCH` / `UNIVERSE` / `BENCHMARK_PROXY` 구분을 유지하도록 SEPA·스캐너·스토리지 병합 로직을 정리하고, 카드/테이블 UI에 소스 배지를 노출.
- **SEPA 판정 강화**: 핵심 7개 코어 기준을 별도 집계해 `7/7=pass`, `6/7=warning`, `5/7 이하=fail` 로직으로 조정하고 추천 등급에서도 core 기준을 반영.
- **VCP 엄밀화**: 수축 단계의 peak/trough 동시 하락 검증을 추가하고, 수축 2개 미만 종목의 점수 상한을 40으로 제한.
- **RS 배치 자동화**: `/api/cron/rs-metrics`가 스케줄 호출 시 자동으로 chunk→finalize를 수행하도록 확장하고, 실패 시 텔레그램 알림 및 Vercel cron 일정을 추가.
- **테스트 보강**: `tests/sepa-core.test.mjs`를 추가하고 VCP 엔진 테스트를 강화해 Week 1 변경을 회귀 테스트로 고정.
