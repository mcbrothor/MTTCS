...
```
변경 대상: app/scanner/page.tsx
연관 파일: lib/finance/core/sepa.ts, lib/finance/engines/vcp/base-pattern-engine.ts, components/scanner/ScanTable.tsx
```
...
- 무엇을: (요청 내용 한 줄)
- 왜: (이유)
- 영향 파일: app/scanner/page.tsx, lib/finance/core/sepa.ts
...
```
sepa.ts를 수정한다면:
  → app/scanner/page.tsx (스캔 결과 표시)
  → app/plan/page.tsx (신규 계획 SEPA 점수)
  → app/watchlist/page.tsx (관심종목 필터)
```
...
- 왜: 부채 높은 종목 걸러내기 위해
- 영향 파일: lib/finance/core/sepa.ts, app/scanner/page.tsx
...
# 4. 특정 파일만 되돌리기
git checkout [커밋 hash] -- lib/finance/core/sepa.ts
```
